...
from centerart_model.umpnet import spherical_sampling
from centerart_model.umpnet.mlp_net import MLP
from centerart_model.umpnet.unet_parts import *
...
class DirModel(nn.Module):
    def __init__(self, num_directions, model_type):
        ...
        self.raw_directions = spherical_sampling.fibonacci(
            num_directions, co_ords="cart"
        )

        image_feature_dim = 256
        action_feature_dim = 128
        output_dim = 1
        self.sgn_action_encoder = MLP(
            3, action_feature_dim, [action_feature_dim, action_feature_dim]
        )
        self.mag_action_encoder = MLP(
            3, action_feature_dim, [action_feature_dim, action_feature_dim]
        )
        ...
        if "sgn" in model_type:
            ...
            self.sgn_image_feature_extractor = MLP(
                512 * 7 * 10, image_feature_dim, [image_feature_dim]
            )
            self.sgn_decoder = MLP(
                image_feature_dim + action_feature_dim,
                3 * output_dim,
                [1024, 1024, 1024],
            )
    ...
    def forward(self, observation, directions=None):
        ...
        if directions is None:
            ...
            for _ in range(observation.size(0)):
                r_mat_T = (
                    R.from_euler("xyz", np.random.rand(3) * 360, degrees=True)
                    .as_matrix()
                    .T
                )
        ...
        torch_directions = torch.from_numpy(directions.astype(np.float32)).to(
            observation.device
        )
        sgn_direction_features = [
            self.sgn_action_encoder(torch_directions[:, i])
            for i in range(num_directions)
        ]
        mag_direction_features = [
            self.mag_action_encoder(torch_directions[:, i])
            for i in range(num_directions)
        ]
        ...
        if "sgn" in self.model_type:
            ...
            for i in range(num_directions):
                feature_input = torch.cat(
                    [sgn_feature, sgn_direction_features[i]], dim=1
                )
        ...
        if "mag" in self.model_type:
            ...
            for i in range(num_directions):
                feature_input = torch.cat(
                    [mag_feature, mag_direction_features[i]], dim=1
                )
...
class UMPModel:
    ...
    def get_direction_affordance(
        self, observations, model_type, torch_tensor=False, directions=None
    ):
        ...
        for id, observation in enumerate(observations):
            ...
            scene_inputs.append(
                np.concatenate(
                    [
                        observation["image"].transpose([2, 0, 1]),
                        observation["image_init"].transpose([2, 0, 1]),
                    ],
                    axis=0,
                )
            )
        ...
        sgn_output, mag_output, skipped_directions = self.dir_model.forward(
            scene_input_tensor.to(self.device_dir), directions=directions
        )  # [B, K, W, H]
        if torch_tensor:
    ...
    def get_position_affordance(self, observations, torch_tensor=False):
        ...
        for observation in observations:
            scene_inputs.append(observation.transpose([2, 0, 1]))
        scene_input_tensor = torch.from_numpy(np.stack(scene_inputs))

        affordance_maps = self.pos_model.forward(
            scene_input_tensor.to(self.device_pos)
        )  # [B, K, W, H]
        if not torch_tensor:
            # TODO: check if this is correct, if no turn it back to 1-softmax
            affordance_maps = 1 - F.softmax(affordance_maps, dim=1)[:, 0]
            # affordance_maps = F.softmax(affordance_maps, dim=1)[:, 0]
            affordance_maps = affordance_maps.data.cpu().numpy()

        # import matplotlib.pyplot as plt

        # affordance_maps = affordance_maps.reshape(480, 640)
        # inverted_cmap = plt.cm.gray_r
        # # Plot the affordance map
        # plt.imshow(affordance_maps, cmap=inverted_cmap, vmin=0, vmax=1)
        # plt.colorbar()
        # plt.show()
