...
from centerart_model.rgb.heatmaps import visualize_heatmap
from centerart_model.rgb.pose_utils import (
    get_poses,
    visualize_poses,
    poses_to_numpy,
    MaskedPoseLoss,
...
def load_rgb_model(ckpt_name: str, ckpt_folder_name: str = "ckpt_rgb", **kwargs):
    ...
    lit_model = LitCenterArtModel.load_from_checkpoint(
        ckpt_path, **kwargs, map_location="cpu"
    ).to(DEVICE)
    return lit_model, rgb_specs


class LitCenterArtModel(pl.LightningModule):
    ...
    def log_predictions(self, rgb, heatmap_out, posemap_out, prefix):
        if self.logger is not None:
            with torch.no_grad():
                ...
                heatmap_pred_vis = visualize_heatmap(
                    np.copy(rgb_np), heatmap_np, with_peaks=True
                )
                posemap_np = poses_to_numpy(posemap_out)
                poses = get_poses(heatmap_np, posemap_np)
                # TODO Assumes same camera instrisics?
                pose_pred_vis = visualize_poses(rgb_np, poses)
                llog = {}
                llog[f"{prefix}/pose"] = wandb.Image(pose_pred_vis, caption=prefix)
                llog[f"{prefix}/heatmap"] = wandb.Image(
                    heatmap_pred_vis, caption=prefix
                )
    ...
    def compute_loss(
        ...
        pose_loss = _pose_loss(
            abs_pose_out, pose_target, heatmap_target, invariance_map
        )
        shape_loss = _mask_l1_loss(latent_emb_out, shape_target, heatmap_target)
        joint_loss = _mse_loss(joint_state_out, joint_target)
        loss = (
            self.specs["heatmap_loss_w"] * heatmap_loss
            + self.specs["pose_loss_w"] * pose_loss
            + self.specs["shape_loss_w"] * shape_loss
            + self.specs["joint_loss_w"] * joint_loss
        )
        return loss, heatmap_loss, pose_loss, shape_loss, joint_loss

    def log_loss(self, loss, heatmap_loss, pose_loss, shape_loss, joint_loss, prefix):
        if self.logger is not None:
            llog = {}
            ...
            llog[f"{prefix}/loss/joint"] = joint_loss
            llog["trainer/global_step"] = self.global_step
            wandb.log(llog)
        return
    ...
    def training_step(self, batch, batch_idx):
        ...
        heatmap_out, abs_pose_out, latent_emb_out, joint_state_out = self.forward(
            rgb, depth
        )
        loss, heatmap_loss, pose_loss, shape_loss, joint_loss = self.compute_loss(
            heatmap_out,
            abs_pose_out,
            latent_emb_out,
            joint_state_out,
        ...
        self.log_loss(loss, heatmap_loss, pose_loss, shape_loss, joint_loss, prefix)
        return loss
    ...
    def configure_optimizers(self):
        ...
        lr_lambda = lambda_learning_rate_poly(
            self.epochs, self.specs["net_config"].optim_poly_exp
        )
        if (
            self.specs["net_config"].optim_warmup_epochs is not None
            and self.specs["net_config"].optim_warmup_epochs > 0
        ):
            lr_lambda = lambda_warmup(
                self.specs["net_config"].optim_warmup_epochs, 0.2, lr_lambda
            )
