...
from centerart_model.sapien.sapien_utils import Obs, CameraObs
from centerart_model.utils.camera import CameraConventions, _normalize, look_at_z
from centerart_model.rgb.rgb_data import RGBDReader
from centerart_model.rgb.pred_postprocessing import get_full_pcd
from centerart_model.gdf.gdf_utils import (
    create_markers_multiple,
    create_markers_multiple_fat,
)
from centerart_model.pipelines.model_pipeline import CenterArtPipeline
from centerart_model.utils.configs import (
    ZED2HALF_PARAMS,
    Directories,
    COLOR_PALETTE,
    CAM_POSE_REAL,
...
def get_pcd_pyrender(pcd_np, color_idx=0):
    sphere_trimesh = trimesh.creation.uv_sphere(radius=0.004)
...
def main(rgb_model: str, idx: int, mode: str = "valid"):
    root_path = Directories.DATA / "CenterArt" / "plot_gif"
    root_path.mkdir(parents=True, exist_ok=True)
    data_loader = RGBDReader(mode)
    centerart_pipeline = CenterArtPipeline(rgb_model, visualize=False)
    ...
    full_pcd = get_full_pcd(rgbd_data)
    ...
    (
        ...
        wTeegoal_list_all,
        wTeegoal_list_best,
    ) = centerart_pipeline.predict_shape_and_grasps(obs)
    ...
    heatmap, _ = centerart_pipeline.rgb_net.get_full_predictions(
    ...
    grasps_pyrender = [
        pyrender.Mesh.from_trimesh(grasps, smooth=False)
        for grasps in all_grasps_trimesh
    ]
    ...
    camera_positions = [
        scene_center + radius * (np.cos(t) * v1 + np.sin(t) * v2) for t in param
    ]
    camera_poses = [
    ...
    def render_gif(scene, camera_poses, filename):
        ...
        images[0].save(
            out_file, save_all=True, append_images=images[1:], duration=50, loop=0
        )
