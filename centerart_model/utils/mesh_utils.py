...
@dataclass
class SceneObject:
    urdf_fpath: pathlib.Path
    id: str = ""
    articulation: sapien.Articulation = None
    pose4x4: np.ndarray = np.eye(4)
    joint_state: float = 0.0
    normalized_joint_state: float = 0.0
    max_joint_state: float = 0.0
    scale: float = 1.0
    link_direction: str = "x"
    emb_index: int = 0
    volume: list = field(default_factory=list)


def load_mesh_o3d(
    path: pathlib.Path, scale: float, mm2m: bool
...
def get_volume_from_mesh(
    mesh_path: pathlib.Path, scale: np.ndarray = np.ones(3)
) -> float:
...
class AmbientCGTexture:
    ...
    def color_fpath(self) -> pathlib.Path:
        path = self.path / f"{self.name}_2K-JPG_Color.jpg"
        return path if path.exists() else None

    @property
    def roughness_fpath(self) -> pathlib.Path:
        path = self.path / f"{self.name}_2K-JPG_Roughness.jpg"
        return path if path.exists() else None

    @property
    def metallic_fpath(self) -> pathlib.Path:
        path = self.path / f"{self.name}_2K-JPG_Metalness.jpg"
        return path if path.exists() else None

    @property
    def normal_fpath(self) -> pathlib.Path:
        path = self.path / f"{self.name}_2K-JPG_NormalGL.jpg"
        return path if path.exists() else None

    @property
    def displacement_fpath(self) -> pathlib.Path:
        path = self.path / f"{self.name}_2K-JPG_Displacement.jpg"
        return path if path.exists() else None
