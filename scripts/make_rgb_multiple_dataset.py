...
def object_collector(collected_objs, json_data, mode):
    mic_flag = False
    high_flag = False
    target_objs = []
...
def main(
    ...
    scene_renderer = SceneImgRenderer(
        headless=True, raytracing=ray_tracing, imgs_per_scene=imgs_per_scene
    )
    ...
    for i in tqdm(range(nr_scenes)):
        ...
        scene_objs = object_collector(collected_objs, json_data, mode)
        poses_raw, y_center, counter_range = get_poses_raw(json_data, scene_objs)
