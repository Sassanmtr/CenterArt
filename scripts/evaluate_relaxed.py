...
def main(
    ...
    for episode in tqdm.tqdm(range(len(rgbd_reader.rgb_paths))):
        ...
        scene_objs_dict = get_obs_test(obj_data, data_dir, urdf_dir, idx)
        camera_pose = scene_objs_dict["wTcam"]
        # transform = sm.SE3(CameraConventions.opencv_T_robotics, check=False)
        ...
        while not (
            ...
            info = behavior.run_relaxed(obs, idx, method)
            print("Run info: ", info)
            grasp_attempts += info["attempts"]
            distances += info["distances"]
            environment.remove_obj()
        print("distances: ", info["distances"])
        log_data = {"grasp_attempts": grasp_attempts, "distances": distances}
        wandb.log(log_data)
    # Save the distances as a numpy array
    file_path = os.path.join(str(Path.cwd() / "results"), f"{method}_gt_dist_mult.npy")
    np.save(file_path, np.array(distances))
    wandb.finish()
    return
...
if __name__ == "__main__":
    ...
    main(
        # rgb_model="x9f0te7z", # multiple objects
        rgb_model="n90qyp90",   # single object
        robot_type="gripper",
